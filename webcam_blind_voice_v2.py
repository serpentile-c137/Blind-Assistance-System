...
Shows bounding boxes with labels, confidence %, and distance in cm
"""
...
# Load Places365 scene recognition model
arch = 'resnet18'
    ...
    with tf.compat.v1.Session(graph=detection_graph) as sess:
        while True:
            ...
            for i in range(min(10, boxes.shape[0])):
                ...
                # Distance estimation in centimeters (approximate)
                apx_distance_m = (1 - (x2 - x1)) ** 4
                apx_distance_cm = round(apx_distance_m * 100, 2)

                # Terminal Output
                print(f"{object_name}: {int(scores[i]*100)}% - {apx_distance_cm:.2f} m")

                # Label Format: "person: 95% - 42.76 cm"
                label = f"{object_name}: {int(scores[i]*100)}% - {apx_distance_cm:.2f} m"
